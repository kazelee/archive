...
/// <summary>
/// 1:1 复刻 RuleOri，除了canBeat和canFollow<br/>
/// 理论上可以在RuleSame中调用RuleOri的函数减少代码复用，算了，不如这样更简单快捷？
/// </summary>
public class RuleSame : IRule {
    ...
    public bool CanBeat(CombData comb1, CombData comb2) {
        ...
        foreach (var kv in types2) {
            if (types1.TryGetValue(kv.Key, out var weight) && weight >= kv.Value) { // 【修改】等于也行
                return true;
            }
        }
    ...
    public bool CanFollowOri(CombData target, List<string> myCardList, out List<CombData> possibleCombs) {
        ...
        foreach (var kv in targetTypes) {
            ...
            foreach (var w in ((Dictionary)TypeCards[kv.Key]).Keys) {
                var wStr = (string)w;
                if (wStr.ToInt() >= kv.Value) { // 【修改】等于也行
                    weightGt.Add(wStr);
                }
