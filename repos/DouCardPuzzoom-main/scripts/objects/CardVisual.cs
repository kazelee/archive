...
public partial class CardVisual : Area2D
{
    ...
    public void SetCardData(CardData cardData) {
        Data = cardData;
        var suitName = CardTool.GetSuitName(Data.SuitNum);
        var pointName = CardTool.GetPointName(Data.PointNum);
    ...
    private void SetTexture(Texture2D texture) {
        ...
        if (IsLastOne) {
            rect.Size = _texture.GetSize();
        }
        else {
            rect.Size = new Vector2(ConstManager.CardSideDistance, _texture.GetSize().Y);
            collider.Position = Vector2.Left * (_texture.GetSize().X / 2 - ConstManager.CardSideDistance / 2f);
        }
        collider.Shape = rect;
        AddChild(collider);
    }
