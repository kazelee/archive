...
/// <summary>
/// 1:1 复刻RuleOri，只需要改canFollow和canBeat的逻辑，出牌无所谓<br/>
/// 炸弹无视此规则？反正王炸肯定是无视的，单牌大小王也是无视的，所以只考虑王牌就行<br/>
/// 直接修改规则为：跟牌必须严格与出牌有同样的花色组成，这样还更好写程序……
/// </summary>
public class RuleSuit : IRule {
    public readonly Dictionary Data = DataLoader.RuleOriData;
    public readonly Dictionary TypeCards = DataLoader.RuleOriTypeCards;
    ...
    public List<CombData> GetAllCombOri(List<string> cardsRule) {
        ...
        foreach (var kv in cardsMap) {
            ...
            if (kv.Value >= 2 && kv.Key != 13 && kv.Key != 14) { // 枚举时不知道大小王可以有多张
                pairList.Add(kv.Key);
        ...
        // 1.2 单顺
        // CombTool.PrintAllCombs(combList);
        CreateStraight(soloList, 5, combList, 1);
        // CombTool.PrintAllCombs(combList);
        ...
        foreach (var trio in trioList) {
            combList.Add(IntList2CombData(new List<int> { trio, trio, trio }));
            // 三带一
            foreach (var card in soloList) {
                // GD.Print("Here");
                if (card == trio) continue;
                // CombTool.PrintAllCombs(combList);
                combList.Add(IntList2CombData(new List<int>() { trio, trio, trio, card }));
                // CombTool.PrintAllCombs(combList);
            }
            // 三带二
        ...
        if (DebugManager.AllowPlane) {
            if (trioChainList != null) {
                ...
                foreach (var intList in trioChainList) {
                    ...
                    // 单张
                    var tmpSolo = soloList.Where(card => !intList.Contains(card)).ToList();
                    
                    // var tmp = tmpSolo.Aggregate("tmpSolo: ", (current, solo) => current + $"{solo}, ");
                    // GD.Print(tmp);
                    
                    // 单张不够用了，直接退出，对子肯定更没戏
                    if (tmpSolo.Count < intList.Count) break;

                    var allSolo = new List<int>();
                    // [Debug]
                    // foreach (var solo in allSolo) {
                    //     GD.Print($"solo: {solo}");
                    // }
                    foreach (var solo in tmpSolo) {
                        // 所有除了三连以外的单牌
                        for (int i = 0; i < cardsMap[solo]; i++) {
                            allSolo.Add(solo);
                        }
                    }
                    
                    // 所有可能的带牌逻辑
                    var possWithsOne = CombineTool.GetCombinations(allSolo, intList.Count);
                    foreach (var possWith in possWithsOne) {
                        var tmpList = new List<int>(chain);
                        tmpList.AddRange(possWith);
        ...
        // 四带
        if (DebugManager.AllowFourWithTwo) {
            foreach (var bomb in bombList) {
                // 四带两单（可以是一对？）
                for (int i = 0; i < soloList.Count; i++) {
                    if (soloList[i] == bomb) continue;
                    for (int j = i + 1; j < soloList.Count; j++) {
                        if (soloList[j] == bomb) continue;
                        combList.Add(IntList2CombData(new List<int> {
                            bomb, bomb, bomb, bomb, soloList[i], soloList[j]
                        }));
                    }
                }
                // 四带两对（可以是同样的对子；直接特殊处理好了）
                for (int i = 0; i < pairList.Count; i++) {
                    // 也可以只有一对
                    if (pairList[i] == bomb) continue;
                    combList.Add(IntList2CombData(new List<int> {
                        bomb, bomb, bomb, bomb, pairList[i], pairList[i]
                    }));
                    // 也可以是一个炸
                    if (cardsMap[pairList[i]] == 4) {
                        combList.Add(IntList2CombData(new List<int> { bomb, bomb, bomb, bomb
                            , pairList[i], pairList[i], pairList[i], pairList[i]}));
                    }
                    // 两对正常情况
                    for (int j = i + 1; j < pairList.Count; j++) {
                        if (pairList[j] == bomb) continue;
                        combList.Add(IntList2CombData(new List<int> { bomb, bomb, bomb, bomb
                            , pairList[i], pairList[i], pairList[j], pairList[j]}));
                    }
                }
            }
        }
    ...
    public bool CanBeat(CombData comb1, CombData comb2) {
        ...
        // 单牌大小王也无视，这个canfollow中就是如此
        if (types2.ContainsKey("solo") && types1.ContainsKey("solo") && types1["solo"] >= 13) {
            return types1["solo"] > types2["solo"];
        }
        
        // 先检查是否严格对应花色
        var comb1SuitList = new List<int>();
        var comb2SuitList = new List<int>();
        foreach (var cd in comb1.Cards) {
            if (cd.SuitNum == SuitNums.Joker) continue;
            if (comb1SuitList.Contains((int)cd.SuitNum)) continue;
            comb1SuitList.Add((int)cd.SuitNum);
        }
        foreach (var cd in comb2.Cards) {
            if (cd.SuitNum == SuitNums.Joker) continue;
            if (comb2SuitList.Contains((int)cd.SuitNum)) continue;
            comb2SuitList.Add((int)cd.SuitNum);
        }

        if (comb2SuitList.Count != 0) { // count = 0 说明对手手牌只有王，硬碰硬（这个时候可以打炸弹）
            // if (comb1SuitList.Count != comb2SuitList.Count) return false;
            // if (comb2SuitList.Any(suitInt => !comb1SuitList.Contains(suitInt))) {
            //     return false;
            // }
            
            // 仅仅考虑，如果我出的牌花色对手没有，不行
            if (comb1SuitList.Any(suitInt => !comb2SuitList.Contains(suitInt))) {
                return false;
            }
        }
    ...
    public bool CanFollow(CombData target, List<CardData> myCardsInHand, out List<CombData> possibleCombs) {
        // 统计别人的花色数，不算大小王
        var targetSuitList = new List<int>();
        foreach (var cd in target.Cards) {
            if (cd.SuitNum == SuitNums.Joker) continue;
            if (targetSuitList.Contains((int)cd.SuitNum)) continue;
            targetSuitList.Add((int)cd.SuitNum);
        }
        
        // 自己的选牌，考虑大小王（所以不用担心王炸的问题）
        var newMyCardsInHand = new List<CardData>();
        foreach (var cd in myCardsInHand) {
            if (cd.SuitNum == SuitNums.Joker || targetSuitList.Contains((int)cd.SuitNum)) {
                newMyCardsInHand.Add(cd);
            }
        }
        
        var myCardList = RuleTool.CardList2RuleList(newMyCardsInHand); // 将符合花色牌型的牌作为选择范围
        if (CanFollowOri(target, myCardList, out var pcs)) {
            var res = new List<CombData>();
            // 必须返回sensitive的样式……
            // 问题：这样只是确保了选牌范围，但仍然可以选择单花色的牌
            // 要不还是把规则改成，哪怕只含对方含的一张也行……
            // 那么housekeeper就应该去寻找花色最纯的那一张牌了……
            foreach (var cb in pcs) {
                var visited = new List<CardData>();
                foreach (var cdStr in cb.RuleList) {
                    var myCards = newMyCardsInHand;
                ...
                IsCombValid(visited, out var toLeadComb);
                res.Add(toLeadComb);
            }
